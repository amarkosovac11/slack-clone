...
export class NotificationService{
  private readonly url='http://localhost:8080/api/notifications';
  ...
  constructor(private readonly http:HttpClient,tokenService:TokenService){this.client=new Client({brokerURL:'ws://localhost:8080/ws',reconnectDelay:5000,beforeConnect:()=>{const token=tokenService.getToken();this.client.connectHeaders=token?{Authorization:`Bearer ${token}`}:{}} ,onConnect:()=>this.subscribe(),onWebSocketClose:()=>{this.subscription=null;}});}
