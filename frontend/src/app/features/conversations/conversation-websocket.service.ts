...
export class ConversationWebSocketService {
  ...
  private desiredConversation: { id: number; userId: number; messageCallback: (event: ConversationMessageEvent) => void; metadataCallback: (event: ConversationMetadataEvent) => void } | null = null;
  ...
  subscribeToConversation(id: number, userId: number, messageCallback: (event: ConversationMessageEvent) => void,
      metadataCallback: (event: ConversationMetadataEvent) => void): void {
    this.unsubscribeConversation();
    this.desiredConversation = { id, userId, messageCallback, metadataCallback }; this.connect(); this.activateMessageSubscription();
  }
  ...
  unsubscribeConversation(): void { this.messageSubscription?.unsubscribe(); this.metadataSubscription?.unsubscribe(); this.messageSubscription = null; this.metadataSubscription = null; this.desiredConversation = null; }
