...
public class WebSocketSecurityInterceptor implements ChannelInterceptor {
    ...
    private void authorizeSubscription(StompHeaderAccessor accessor) {
        ...
        matcher = USER_NOTIFICATION_TOPIC.matcher(safeDestination);
        if (matcher.matches()) {
            var user = conversationAccessService.requireUser(accessor.getUser().getName());
            if (!user.getId().equals(Long.valueOf(matcher.group(1)))) throw new IllegalArgumentException("Cannot subscribe to another user's notifications");
            return;
        }
